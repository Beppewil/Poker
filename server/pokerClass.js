...
class PokerGame {
      ...
      if (!this.everyonePlaying) {
        if (this.timeLeft > 0) {
          if ((Object.keys(this.players).filter(key => this.players[key].playing == true).length) / Object.keys(this.players).length >= 0.5 && Object.keys(this.players).length > 2) {
            ...
            this.io.to(this.roomID).emit("updateTimer", this.timeLeft, true);
            over = true;
          }
        } else {
          console.log("new round");
  ...
  roundEnd() {
    try {
      ...
      for (let player in this.players) {
        ...
        tempCards.splice(tempCards.indexOf(this.players[player].hand), 1); // Removing the player's cards from the array
      ...
      for (let sidepot in sidepots) {
        var elligablePlayers = [];
        if (sidepot == 0) {
          winningStatement += "Main Pot " + (parseInt(sidepots[sidepot].pot)) + " winners: ";
        } else {
  ...
  getPlayers() {
    try {
      ...
      for (let player in this.players) {
        gotPlayers.push({
          playerNum: this.players[player].playerNum,
          username: this.players[player].username,
          money: this.players[player].money,
          folded: this.players[player].folded,
          playing: this.players[player].playing,
          bet: this.players[player].bet,
          roundBet: this.players[player].roundBet,
          lastAction: this.players[player].lastAction
        });
      }
      return gotPlayers;
    } catch (error) {
